...
namespace Assignment1
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var qes1 = emp.Where(e => e.DOJ < new DateTime(2015, 1, 1));
            foreach (var joindate in qes1)
            { 
                Console.WriteLine($"{joindate.FirstName} {joindate.LastName}");
            }

            Console.WriteLine("---------2)Employees whose DOB is after 1/1/1990----------");
            var qes2 = emp.Where(e => e.DOB > new DateTime(1990, 1, 1));
            foreach (var DOB in qes2)
            { 
                Console.WriteLine($"{DOB.FirstName} {DOB.LastName}"); 
            }

            Console.WriteLine("--------3)Employees whose title is Consultant or Associate--------");
            var qes3 = emp.Where(e => e.Title == "Consultant" || e.Title == "Associate");
            foreach (var title in qes3)
            {
            ...
            var qes9 = emp.GroupBy(e => e.City)
                       .Select(g => new { City = g.Key, Count = g.Count() });
            foreach (var Groupbycity in qes9)
            {
            ...
            var qes10 = emp.GroupBy(e => new { e.City, e.Title })
                       .Select(g => new { g.Key.City, g.Key.Title, Count = g.Count() });
            foreach (var group in qes10)
            {
