...
class Emp
{
    public static void Main(string[] args)
    {
        ...
        var Emp = from emp in emplist select emp;
        ...
        var place = from emp in emplist
                          where emp.city != "Mumbai"
                          select emp;
        Console.WriteLine("\nEmployee Not in Mumbai:");
        foreach (var emp in place)
        {
            Console.WriteLine($"ID: {emp.EmployeeID}, Name: {emp.First_Name} {emp.Last_Name}, Title: {emp.title}, DOB: {emp.DOB.ToShortDateString()}, DOJ: {emp.DOJ.ToShortDateString()}, City: {emp.city}");
        }

        
        var Am = from emp in emplist
                             where emp.title == "AsstManager"
                             select emp;
        Console.WriteLine("\nEmployee with Title 'AssistManager':");
        foreach (var emp in Am)
        {
            Console.WriteLine($"ID: {emp.EmployeeID}, Name: {emp.First_Name} {emp.Last_Name}, Title: {emp.title}, DOB: {emp.DOB.ToShortDateString()}, DOJ: {emp.DOJ.ToShortDateString()}, City: {emp.city}");
        }

        
        var Lastname = from emp in emplist
                                  where emp.Last_Name.StartsWith("S")
                                  select emp;
        Console.WriteLine("\nEmployees  Last Name Starting with 'S':");
